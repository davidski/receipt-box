...
import { entryCsv, entryExportHeaders, entryValues } from '../utils/csv-export'
import { canonicalRecord, cleanHeader } from '../utils/import-record'
...
type ManagedCategory = { id: string, name: string, itemCount: number, entryCount: number }
type ItemVariant = { name: string, uses: number, lastUsed: string, category?: string | null }
...
} = await useFetch<ManagedCategory[]>(apiUrl('/categories/manage'), { immediate: false, server: false })
const {
  data: duplicateItems,
...
const renameItemCategory = ref<string | null>(null)
const originalItemCategory = ref<string | null>(null)
...
function offerCategoryResolution(error: any, target: string, retry: (category: string) => Promise<void>) {
  const categories = error?.data?.data?.categories
  if (!Array.isArray(categories) || !categories.length || !categories.every(value => typeof value === 'string')) return false
  conflictCategory.value = undefined
  categoryConflict.value = { target, categories, retry }
  return true
}

async function resolveCategoryConflict() {
  if (!categoryConflict.value || !conflictCategory.value) return
  const { retry } = categoryConflict.value
  const category = conflictCategory.value
  categoryConflict.value = null
...
const categoryWillChange = computed(() => renameItemCategory.value !== originalItemCategory.value)
...
const itemEditButtonLabel = computed(() => {
  if (nameWillChange.value && categoryWillChange.value && packageWillChange.value) return 'Apply item changes'
  if (nameWillChange.value && categoryWillChange.value) return `Rename and recategorize ${selectedItemUses.value} ${selectedItemUses.value === 1 ? 'entry' : 'entries'}`
  if (categoryWillChange.value && packageWillChange.value) return 'Update category and package details'
  if (categoryWillChange.value) return `Update category for ${selectedItemUses.value} ${selectedItemUses.value === 1 ? 'entry' : 'entries'}`
...
watch(activeSection, (section) => {
  if (['categories', 'items'].includes(section) && categoriesStatus.value === 'idle') {
    void loadCategories()
  }
...
function categoryTargetOptions(source: ManagedCategory) {
  return (categories.value || []).filter(category => category.id !== source.id).map(category => ({
    label: category.name,
    value: category.id
  }))
}
...
async function saveCategory(category: ManagedCategory) {
  ...
  if (!name) return
  categoryBusyId.value = category.id
  categoryError.value = ''
  try {
    await $fetch(apiUrl(`/categories/${category.id}`), { method: 'PATCH', body: { name } })
    editingCategoryId.value = null
    await refreshCategories()
  } catch (error: any) {
    categoryError.value = storeErrorMessage(error, 'Could not rename the category')
  } finally {
...
function requestCategoryMerge(source: ManagedCategory) {
  ...
  requestConfirmation({
    ...
    description: `Merge “${source.name}” into “${target.name}”? This will update ${source.itemCount} ${source.itemCount === 1 ? 'item' : 'items'} and ${source.entryCount} unique purchase ${source.entryCount === 1 ? 'entry' : 'entries'} to use ${target.name}.`,
    confirmLabel: 'Merge categories',
    confirmColor: 'warning',
    action: async () => {
      categoryBusyId.value = source.id
      categoryError.value = ''
      try {
        await $fetch(apiUrl('/categories/merge'), { method: 'POST', body: { sourceId: source.id, targetId: target.id } })
        await refreshCategories()
      } catch (error: any) {
        categoryError.value = storeErrorMessage(error, 'Could not merge the categories')
      } finally {
        categoryBusyId.value = null
      }
    }
...
function requestCategoryDelete(category: ManagedCategory) {
  requestConfirmation({
    title: 'Delete category?',
    description: `Delete “${category.name}”? This will remove it from ${category.itemCount} ${category.itemCount === 1 ? 'item' : 'items'} and ${category.entryCount} unique purchase ${category.entryCount === 1 ? 'entry' : 'entries'}, leaving those purchases Uncategorized.`,
    ...
    action: async () => {
      ...
      categoryError.value = ''
      try {
        await $fetch(apiUrl(`/categories/${category.id}`), { method: 'DELETE' })
        await refreshCategories()
      } catch (error: any) {
        categoryError.value = storeErrorMessage(error, 'Could not delete the category')
      } finally {
        categoryBusyId.value = null
      }
    }
...
async function addCategory() {
  ...
  categoryError.value = ''
  try {
    await $fetch(apiUrl('/categories'), { method: 'POST', body: { name } })
    newCategoryName.value = ''
    await refreshCategories()
  } catch (error: any) {
    categoryError.value = storeErrorMessage(error, 'Could not add the category')
  } finally {
...
async function mergeItemGroupConfirmed(group: ItemDuplicateGroup, target: string, sources: string[], category?: string) {
  ...
  try {
    const result = await $fetch<{ target: string, mergedNames: number, mergedEntries: number }>(apiUrl('/items/merge'), {
      method: 'POST',
      body: { target, sources, ...(category === undefined ? {} : { category }) }
    })
    ...
    if (offerCategoryResolution(error, target, choice => mergeItemGroupConfirmed(group, target, sources, choice))) return
...
watch(renameItemSource, async (source) => {
  ...
  const item = duplicateItems.value?.items.find(candidate => candidate.name === source)
  originalItemCategory.value = item?.category ?? null
  renameItemCategory.value = originalItemCategory.value
...
async function renameItem() {
  ...
  if (!source || !target || (!nameWillChange.value && !categoryWillChange.value && !packageWillChange.value)) return
  ...
  const changes = [
    ...
    categoryWillChange.value ? `set the category for all purchases of “${target}” to “${renameItemCategory.value || 'Uncategorized'}”` : '',
...
async function renameItemConfirmed(source: string, target: string, category?: string) {
  ...
  try {
    ...
    const categoryChange = category === undefined ? (categoryWillChange.value ? renameItemCategory.value : undefined) : category
    const result = await $fetch<{ target: string, renamedEntries: number, updatedEntries: number }>(apiUrl('/items/rename'), {
      method: 'PATCH', body: { source, target, dimensions, ...(categoryChange === undefined ? {} : { category: categoryChange }) }
    })
    itemMergeNotice.value = [
      result.renamedEntries ? `${result.renamedEntries} ${result.renamedEntries === 1 ? 'entry was' : 'entries were'} renamed to ${result.target}` : '',
      categoryChange !== undefined ? `Category for ${result.target} was set to ${categoryChange || 'Uncategorized'} for all item history` : '',
    ...
    await refreshDuplicateItems()
    if (categoryChange !== undefined) await refreshCategories()
  } catch (error: any) {
    if (offerCategoryResolution(error, target, choice => renameItemConfirmed(source, target, choice))) return
...
async function exportXlsx() {
  ...
  try {
    ...
    sheet.columns = [
      { width: 15 }, { width: 34 }, { width: 22 }, { width: 20 }, { width: 15 }, { width: 15 },
...
<template>
  <div class="w-full max-w-[1200px] mx-auto">
    ...
    <UModal
      :open="Boolean(categoryConflict)"
      title="Choose a category"
      :description="`These items have different categories. Choose the category for all purchases of ${categoryConflict?.target || 'the merged item'}.`"
      @update:open="!$event && (categoryConflict = null)"
    >
      <template #body>
        <UFormField label="Category">
          <USelectMenu v-model="conflictCategory" :items="categoryConflict?.categories || []" placeholder="Choose a category…" class="w-full" />
        </UFormField>
      </template>
      <template #footer>
        <div class="flex w-full justify-end gap-2">
          <UButton label="Cancel" color="neutral" variant="ghost" @click="categoryConflict = null" />
          <UButton label="Apply changes" :disabled="!conflictCategory" @click="resolveCategoryConflict" />
        </div>
      </template>
    </UModal>
